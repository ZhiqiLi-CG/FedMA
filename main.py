...
def check_zero_rate(arr):
    total_number=int(np.prod(arr.shape))
    count = np.count_nonzero(arr)
    return 1-count/total_number

def BBP_MAP(nets_list, model_meta_data, layer_type, net_dataidx_map, 
                            averaging_weights, args, 
                            device="cpu",prune_rate=0):
    # starting the neural matching
    models = nets_list
    cls_freqs = traindata_cls_counts
    n_classes = args_net_config[-1]
   ...
   # print(len(batch_weights[0]))
    #print("batch_weights",batch_weights)
    #print("===================================================")
    #print("===================================================")
    #exit(0)
   # for i in range(len(nets_list)):
    #    for j in range(0,len(batch_weights[0]),2):
     #       print(type(batch_weights[i][j]))
            #print(layer_type[i][j])
    ...
    for layer_index in range(1, n_layers):
        ################# Now we Pruning the layer_index layers
        for i in range(len(nets_list)):
            abs_array = np.abs(batch_weights[i][2 * layer_index - 2])
            mask = np.ones(abs_array.shape,  dtype=bool)
            k = int(np.prod(mask.shape) *prune_rate)
            idx = np.argpartition(abs_array.reshape(-1), k)[:k]
            mask.reshape(-1)[idx] = 0
            batch_weights[i][2 * layer_index - 2]=batch_weights[i][2 * layer_index - 2]*mask
            print(prune_rate,"check_zero_rate",check_zero_rate(batch_weights[i][2 * layer_index - 2]))
...
def fedma_comm(batch_weights, model_meta_data, layer_type, net_dataidx_map, 
    ...
    for cr in range(comm_round):
        ...
        for worker_index in range(args.n_nets):
            ...
            print(retrained_cnn)
        # BBP_MAP step
        hungarian_weights, assignments_list = BBP_MAP(retrained_nets, model_meta_data, layer_type, net_dataidx_map, averaging_weights, args, device=device,prune_rate=prune_rate)
...
if __name__ == "__main__":
    ...
    if args.comm_type == "fedavg":
        ...
        fedma_comm(comm_init_batch_weights,
                                 ...
                                 assignments_list,
                                 comm_round=args.comm_round,
                                 device=device,prune_rate=args.prune_rate)